...
class L7FlowTracing(Base):
    ...
    async def query_and_trace_flowmetas(
            ...
            ntp_delay_us: int = 10000) -> Tuple(list, list):
        ...
        for i in range(max_iteration):
            ...
            # 2.1. C2: 以 req_tcp_seq & resp_tcp_seq 作为条件查询关联 flow
            tcp_seq_filters = []
            if new_req_tcp_seqs:
            ...
            # 2.2. C3: 以 syscall_trace_id_request & syscall_trace_id_response 作为条件查询关联 flow
            ...
            # 2.3. C3: 以 x_request_id_0 & x_request_id_1 作为条件查询关联 flow
    ...
    async def trace_l7_flow(self,
        ...
        l7_flows_merged, app_flows, networks, flow_index_to_ids, related_flow_index_map = sort_all_flows(
        ...
        return format_final_result(l7_flows_merged, networks, app_flows,
                                   ...
                                   flow_index_to_ids, related_flow_index_map)
...
class TraceInfo:
    def __init__(self, _id, vtap_id, _type, start_time_us, end_time_us,
                 req_tcp_seq, resp_tcp_seq, trace_id, span_id, parent_span_id,
                 x_request_id_0, x_request_id_1, syscall_trace_id_request,
                 syscall_trace_id_response, origin_flow_list,
...
def merge_flow(flows: list, flow: dict) -> bool:
    ...
    need_compare_request_id = flow[
        'request_id'] and flow['l7_protocol'] != L7_PROTOCOL_MYSQL
...
def sort_all_flows(dataframe_flows: DataFrame, network_delay_us: int,
    ...
    set_all_relate(trace_infos, related_flow_index_map, network_delay_us)  # XXX: flow function
    # 构建一个 flow._index 到 flow._id(s) 的映射，方便后续 related_flow_index_map 的使用
    flow_index_to_ids = [None] * len(flows)
    for flow in flows:
        flow_index_to_ids[flow['_index']] = flow['_id']
    ...
    return services, app_flows, networks, flow_index_to_ids, related_flow_index_map
...
def calculate_related_ids(
    response, flow_index_to_ids: list,
    related_flow_index_map: defaultdict(inner_defaultdict_set)):
    """
    ...
    for flow in return_flows:
        ...
        for _index, related_types in related_flow_index_map[
            ...
            for _id in flow_index_to_ids[_index]:
...
def format_final_result(
    ...
    calculate_related_ids(response, flow_index_to_ids, related_flow_index_map)  # XXX: slow function
