...
6. 针对 WebSphereMQ 异步消息场景的特殊连接：client 侧叶子 NetworkSpanSet → server 侧根 NetworkSpanSet
...
7. 弱关联：client 侧叶子 NetworkSpanSet → server 侧根 NetworkSpanSet（通过 trace_id 推断）
- **仅在配置项 `span_set_connection_strategies` 包含 `net_span_c_to_s_via_trace_id` 时生效**
- 此场景在场景 1-6 全部执行并清空匹配状态后，作为独立的弱关联阶段单独运行，不干扰准确连接
- 连接条件：
  - client 侧叶子 Span（`tap_side` 以 `c` 开头）无子节点（`children_count == 0`）
  - server 侧根 Span（`tap_side` 以 `s` 开头）无父节点（`parent_id < 0`），且 `is_net_root` 为真
  - 双方 trace_id 有交集
